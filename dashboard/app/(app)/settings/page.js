...
// Is an env var set to a non-empty value? Used only for ●/○ status — values are never sent to the client.
const has = (k) => {
...
export default function Settings() {
  ...
  // Integration readiness the dashboard can see (its own container env). Non-secret descriptors
  // (public URLs, ids) are shown; secrets are reduced to "set / not set".
  const integrations = [
  ...
  // Worker/renderer-side config lives in those containers' env, not the dashboard's — reference only.
  const workerEnv = [
